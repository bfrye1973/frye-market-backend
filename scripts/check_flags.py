#!/usr/bin/env python3
import json, sys
from scripts.build_outlook_source_from_polygon import (
    fetch_range_daily, compute_intraday_flags, date_str
)
...
def check(symbol="AAPL"):
    ...
    c1 = closes[-1] if len(closes)>=1 else None
    c2 = closes[-2] if len(closes)>=2 else None

    # Fake “today” using last bar (EOD parity test)
...
if __name__ == "__main__":
    for sym in (sys.argv[1:] or ["AAPL","MSFT","XOM","JPM","XLU"]):
        check(sym)
