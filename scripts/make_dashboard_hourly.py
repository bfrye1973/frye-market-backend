...
# -*- coding: utf-8 -*-
"""
Ferrari Dashboard — make_dashboard_hourly.py (1-Hour Market Meter, Stage 1 • Fast Valuation + 1h crosses)

- EMA posture: EMA 8/18 (faster than 10/20)
- SMI: %K=12, %D=7, EMA=5 (kept; we can tighten to 12/5/5 later if desired)
- Accel thresholds: ±1.5 (ΔBreadth1h + ΔMomentum1h proxy)
- Squeeze 1h published as Expansion% (0=tight, 100=expanded) per your v1 contract
- Mirrors cross info into metrics/hourly (no schema changes)
- After-hours soft freeze suggestion left to downstream consumer
"""

from __future__ import annotations
import argparse, json, os, sys, time, urllib.request, math
...
W_EMA, W_MOM, W_BR, W_SQ, W_LIQ, W_RISK = 40, 25, 15, 10, 10, 5   # Breadth=15 for quicker rotation
FULL_EMA_DIST = 0.60

W_EMA_1H, W_SMI1H, W_SMI4H = 0.60, 0.20, 0.20  # cap SMI4h ≤ 0.20
...
def smi_kd(H: List[float], L: List[float], C: List[float],
    ...
    for i in range(n):
        ...
        kv.append(max(-100.0, min(100.0, val)))
...
def groups_to_sector_cards(groups: Dict[str, dict]) -> List[dict]:
    ...
    alias={"healthcare":"health care","health-care":"health care","info tech":"information technology","technology":"information technology",
           "tech":"information technology","communications":"communication services","comm services":"communication services","telecom":"communication services",
    ...
    for name,g in (groups or {}).items():
        ...
        m = 0.0 if (u+dn:=u+d)==0 else round(100.0*u/(u+dn),2)
...
def build_hourly(source_js: Optional[dict], hourly_url: str) -> dict:
    ...
    NH=NL=UP=DN=0.0
    for c in cards or []:
        NH+=float(c.get("nh",0)); NL+=float(c.get("nl",0)); UP+=float(c.get("up",0)); DN+=float(c.get("down",0))
    ...
    for c in cards or []:
        ...
        if isinstance(bp,(int,float)): total+=1; good+= 1 if bp>50.0 else 0
    ...
    # 3a) Breadth 1h (fast proxy same as v1)
    a_fast=b_fast=None
    # (if you feed ETFs 1h in the future, compute %EMA8>EMA18 & %close>open here)

    # Blend fallback
    breadth_1h = breadth_slow

    # 3b) Momentum 1h legacy proxy (kept for deltas)
    momentum_1h_legacy = momentum_slow

    # 3c) SPY 1h Momentum combo (EMA posture + SMI1h + SMI4h)
    ...
    # 3d) Squeeze 1h (Expansion %) per v1
    squeeze_1h = 50.0
    if len(spy_1h) >= 6:
        H=[b["high"] for b in spy_1h]; L=[b["low"] for b in spy_1h]; C=[b["close"] for b in spy_1h]
    ...
    print(f"[1h] breadth_1h={breadth_1h} mom_combo_1h={momentum_combo_1h} sq_1h(Exp)={squeeze_1h} liq_1h={liquidity_1h} vol_1h_scaled={out['metrics']['volatility_1h_scaled']}")
    return out
