...
GOAL
- Restore 10-minute job runtime to last week's behavior (<~15–20 min), no surprises.

KEY CHOICES
- NO ADR momentum in intraday path (that was the slowdown).
- Snapshots batched by 100, light throttle (0.10s).
- Simple retry (max 3, 0.5s) — no exponential backoff that stacks up.
- Hourly/Daily keep simple NH/NL/3U/3D from completed bars.
- Daily Squeeze = Lux PSI (SPY, conv=50, length=20). Intraday squeeze (fuel) unchanged.

OUTPUT (matches your working schema)
{
  "timestamp": "…Z",
  "mode": "intraday10" | "hourly" | "daily",
  "groups": {
    "<Sector>": { "nh":int, "nl":int, "u":int, "d":int,
                  "vol_state":"Mixed", "breadth_state":"Neutral", "history":{"nh":[]}}
  },
  "global": {
    "squeeze_pressure_pct": int(0..100),     # intraday PSI ("fuel")
    "squeeze_state": "none"|"on"|"firingUp"|"firingDown",
    "daily_squeeze_pct": float(0..100),      # Lux PSI on DAILY SPY (2 decimals)
    "volatility_pct": int(0..100),           # SPY ATR% percentile
...
def fetch_hourly(ticker: str, hours_back: int = 72) -> List[Dict[str, Any]]:
    ...
    start = end - timedelta(days=max(7, hours_back // 6 + 2))  # buffer days
...
def discover_sectors() -> Dict[str, List[str]]:
    if not os.path.isdir(SECTORS_DIR):
        raise SystemExit(f"Missing {SECTORS_DIR}. Add CSV files like data/sectors/Tech.csv (header 'Symbol').")
    sectors={}
...
def watermarks_last_10d(symbols: List[str]) -> Dict[str, Tuple[float|None,float|None,float|None,float|None]]:
    ...
    for i,sym in enumerate(symbols):
        ...
        if (i+1)%20==0: time.sleep(0.08)  # tiny breath every 20 symbols
    return out
...
def lux_psi_from_closes(closes: List[float], conv: int = 50, length: int = 20) -> float | None:
    """
    Lux Squeeze PSI (correlation of log(span) vs time).
    Returns 0..100 (higher = tighter).
    """
...
def volatility_pct_spy() -> int:
    ...
    # build short history of ATR% to rank
    history=[]
...
def build_counts_daily(symbols: List[str]) -> Dict[str,int]:
    c={"nh":0,"nl":0,"u":0,"d":0}
    for i,sym in enumerate(symbols):
        bars = fetch_daily(sym, 22)[-12:]  # just enough for flags
...
def build_counts_hourly(symbols: List[str]) -> Dict[str,int]:
    c={"nh":0,"nl":0,"u":0,"d":0}
    for i,sym in enumerate(symbols):
        bars_h = fetch_hourly(sym, hours_back=120)[-12:]  # last 12 hours
...
def build_counts_intraday10(symbols: List[str]) -> Dict[str,int]:
    ...
    - NO per-symbol daily ADR work (this was the slowdown)
    """
    c={"nh":0,"nl":0,"u":0,"d":0}
...
# ---------------- MAIN ----------------
def main():
    ap = argparse.ArgumentParser(description="Build outlook_source.json (intraday10/hourly/daily) — FAST")
