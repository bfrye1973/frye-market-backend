...
- Emits complete gauges + summary
- Option-A aliases for your React cluster
- Accepts both `sectors` (list) and legacy `groups` (dict)
- Adds meta.pipeline env tag
"""
...
def score_and_verdict(b,m,f,w,o):
    s = 0.35*b + 0.35*m + 0.15*f + 0.075*o + 0.075*(100-w)
    s = int(round(s))
    if   s>=70: v="Risk-On"
...
def extract(src: Dict[str,Any]) -> Dict[str,Any]:
    ...
    fuel_pct  = to_pct(src.get("squeeze_pressure_pct") or src.get("global",{}).get("squeeze_pressure_pct") or raw_psi, True)
    ...
    return {"sectors":sectors,"fuel_pct":fuel_pct,"water_pct":water_pct,"oil_pct":oil_pct,"psi_value":psi_value,"squeeze_state":squeeze_state}
...
def lights_and_bullets(b,m,f,w,o,verdict,stale,sq,psi):
    ...
    lights={"risk_off": verdict in ("Caution","Risk-Off"), "low_fuel": f<=FUEL_LOW_PCT, "low_oil": o<=OIL_LOW_PCT,
            "overheat": w>=WATER_OVERHEAT_PCT, "stale_data": bool(stale), "breadth": ring(b), "momentum": ring(m)}
    bullets=[f"Breadth {int(round(b))}/100; Momentum {int(round(m))}/100."]
    if psi is not None:
        bullets.append(f"Squeeze: {sq}; Fuel PSI {psi:.2f if 0.0<=psi<=1.0 else int(round(psi))} (~{int(round(f))}/100).")
    else:
...
def main():
    ...
    src = jload(args.source) or {}
    prev= jload(args.out) or {}
    s = extract(src)
    ...
    fuel=s["fuel_pct"]; water=s["water_pct"]; oil=s["oil_pct"]
    ...
    lights, bullets = lights_and_bullets(b["pct"],m["pct"],fuel,water,oil,verdict,stale,s["squeeze_state"],s["psi_value"])
    now = datetime.now(timezone.utc).isoformat()
    gauges={"rpm":b,"speed":m,
            "fuel":{"pct":fuel,"psi":s["psi_value"],"state":s["squeeze_state"],"label":"Squeeze"},
    ...
    odos={"breadth_net":b["raw"]["netNHNL"],"momentum_net":m["raw"]["net3U3D"],
          "squeeze_psi": s["psi_value"] if s["psi_value"] is not None else round(fuel,2),
          "breadthOdometer":b["raw"]["netNHNL"],"momentumOdometer":m["raw"]["net3U3D"],
          "squeeze": s["psi_value"] if s["psi_value"] is not None else round(fuel,2)}
