#!/usr/bin/env python3
"""
Ferrari Dashboard — make_dashboard.py (R1.2, Option-A + groups adapter)

- Always emits complete gauges (rpm/speed/fuel/water/oil) and summary (score, verdict, bullets)
...
def main():
    ...
    summary = {
        ...
        "sectors": {  # quick roll-up counts for UI
            "total": len(s["sectors"]) if isinstance(s["sectors"], list) else 0,
            "upBreadth": None, "upMomentum": None  # optional; not used by gauges
        }
    }
    out = {
        "schema_version": SCHEMA_VERSION,
        "updated_at": now_iso,
        "meta": {"ts": now_iso, "version": "1.2"},
        "gauges": gauges,
        "odometers": odometers,
        "lights": lights,
        "summary": summary,
        "outlook": {
            "sectorCards": source.get("sectorCards") or []
        }
    }
