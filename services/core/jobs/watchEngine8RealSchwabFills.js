...
//
// Active CME-style session cadence defaults to 10 seconds.
...
function cmeSessionActive(date = new Date()) {
  ...
  // Monday-Thursday: treat the daily 17:00-18:00 ET maintenance hour as quiet.
...
console.log(JSON.stringify({
  engine: "engine8.schwabRealFillWatcher.v1",
...
while (true) {
  ...
  try {
    const out = await observeSchwabRealFills({ now: startedAt });
    console.log(JSON.stringify({
      ...
      futuresFillsNormalized: out.futuresFillsNormalized,
