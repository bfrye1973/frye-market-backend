...
// - Institutional zones are ONLY strength >= 90.
// - Weak institutionals (strength 60–89) are NOT institutional zones.
// - Weak institutionals (60–89) become shelves.
//
// ✅ TEMP CLEAN RULE (KEEP, BUT FIXED TO USE TRUE INSTITUTIONALS ONLY):
// - Remove shelves overlapping sticky institutional structures, but ONLY when those structures have strength >= 90.
// - Negotiated zones (|NEG|) never count as institutional for this filter.
//
// ✅ NEW LOCKED RULE (PER USER):
...
// overlap ratio (used for institutional overlap rule)
...
//
// ✅ FIXED: only treat strength>=90 structures as "institutional"
// ✅ Negotiated (|NEG|) never counts as institutional
...
// ---------------- NEW: Weak institutionals -> shelves ----------------
//
...
function loadWeakInstitutionalsAsShelves() {
  ...
  try {
    ...
    for (const z of arr) {
      ...
      // Try to map to Acc/Dist if possible. Default Acc if unknown.
      ...
      out.push({
        ...
        comment: `Converted weak institutional (${round2(strengthRaw)})`,
        locked: false,
    ...
    console.log(`[SHELVES] Weak institutionals converted to shelves: ${out.length}`);
    return out;
  } catch (e) {
    console.warn(`[SHELVES] Failed to convert weak institutionals: ${e?.message}`);
    return [];
  }
}
...
async function main() {
  try {
    ...
    // ✅ TRUE Institutional ranges only (strength >= 90, not NEG)
    ...
    // 2) Weak institutionals (60–89) become shelves
    const weakInstShelvesAll = loadWeakInstitutionalsAsShelves();
    const weakInstInBand = filterShelvesToBand(weakInstShelvesAll, currentPriceAnchor, BAND_POINTS);
    ...
    console.log("[SHELVES] Weak institution shelves kept (band):", weakInstInBand.length);
    ...
    // 6) Merge shelves: manual + auto + weak institutionals
    const mergedRaw = [...manualInBand, ...autoMapped, ...weakInstInBand];
    ...
    console.log(
      `[SHELVES] Emitting shelves: manual ${manualInBand.length} + weakInst ${weakInstInBand.length} + auto ${autoMapped.length} => merged ${mergedRaw.length} => afterInstitutionFilter ${mergedNoInstitution.length} (removed ${removedByInstitution}) => collapsed ${collapsed.length} => capped ${capped.length} => final ${finalLevels.length}`
    );
    ...
    const payload = {
      ...
      meta: {
        ...
        weak_institutionals: {
          enabled: true,
          rule: "structures_sticky strength 60–89 (non-NEG) become shelves",
          in_band: weakInstInBand.length,
        },
