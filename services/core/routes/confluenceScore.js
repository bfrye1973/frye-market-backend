...
/**
 * ✅ Keep Strategy section alive:
 * - If scorer returns invalid due to NO_ZONE_NO_TRADE, convert to "waiting"
 * - Always include Engine 2 fib anchors/tag in a stable place (engine2 + context.fib)
 */
...
confluenceScoreRouter.get("/confluence-score", async (req, res) => {
  ...
  try {
    ...
    /* ----------------------------
       Engine 2 fib (always attempt; degrade safely)
       ---------------------------- */
    const fibUrl =
    ...
    /* ----------------------------
       Engine 3 (reaction) — degrade safely
       ---------------------------- */
    const e3Url =
    ...
    /* ----------------------------
       Engine 4 (volume) — degrade safely
       ---------------------------- */
    let volume = null;
    let volumeState = "NO_ACTIVE_ZONE";

    if (execZoneRef && zoneLo != null && zoneHi != null) {
      // Your backend-1 runs on :10000 internally; keep this overrideable
    ...
    // Keep alive when not in zone (no more “shutdown”)
    keepAliveNoZone(out);

    /* ----------------------------
       Attach stable fields + SAFE context (no circular refs)
       ---------------------------- */
    ...
    // ✅ Engine 2 promoted fields for Strategy UI (this is what you asked for)
    out.engine2 = out.engine2 || {};
    out.engine2.anchorTag = fib?.signals?.tag ?? null; // "W2" etc
    out.engine2.invalidated = fib?.signals?.invalidated ?? false;
    out.engine2.inRetraceZone = fib?.signals?.inRetraceZone ?? false;
    out.engine2.near50 = fib?.signals?.near50 ?? false;
    out.engine2.request = { tf, degree, wave };

    // SAFE context for UI
    out.context = out.context || {};

    // Active zone ref used for engines
    ...
    // ✅ Fib included always (anchors for Elliott marks)
    // NOTE: We only include safe subsets (no huge diagnostics objects if you don’t want)
    out.context.fib = {
      meta: fib?.meta ?? null,
    ...
    // ✅ Reaction summary (safe)
    out.context.reaction = {
    ...
    // ✅ Volume summary (safe) — NO raw object stored (prevents circular JSON)
    out.context.volume = {
    ...
    // Light Engine 1 context for UI (safe)
    out.context.engine1 = {
