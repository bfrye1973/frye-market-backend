...
function computeTrendVsWave({
  ...
  // ------------------------------------------------------------
  // Case 5:
  // Higher timeframe remains strong, but lower timeframe / 1H is pulling back.
  // This is the exact "no blind short" state:
  // no long yet, but do not blindly short against strong Daily/4H support.
  // ------------------------------------------------------------
  if (
    (minorPhase === "IN_W3" || minorPhase === "IN_W5") &&
    (minutePhase === "IN_W2" || minutePhase === "IN_W4") &&
    fourHourSupportive &&
    dailyStrong &&
    masterSupportive &&
    priceAboveDailyEma10 === true &&
    priceAbove4hEma10 === true &&
    (
      oneHourWeak ||
      priceAbove1hEma10 === false ||
      structureState === "FAILURE"
    ...
    return {
      state: "HTF_STRONG_LTF_PULLBACK",
      waveSuggestion: "MINUTE_PULLBACK_INSIDE_HIGHER_IMPULSE",
      trendConfirmation: "HIGHER_TIMEFRAME_STILL_SUPPORTED",
      marketMeterBias: "BULLISH_PULLBACK",
      oneHourScore,
      fourHourScore,
      dailyScore,
      masterScore,
      ...
      confirmedMinutePhase,
      reasonCodes: [
        "HTF_STRONG_LTF_PULLBACK",
        `${minorPhase}_HIGHER_IMPULSE`,
        `${minutePhase}_LOWER_TIMEFRAME_PULLBACK`,
        "FOUR_HOUR_SCORE_SUPPORTIVE",
        "DAILY_SCORE_STRONG",
        masterScore === null ? "MASTER_SCORE_UNAVAILABLE" : "MASTER_SCORE_SUPPORTIVE",
        "DAILY_EMA10_HOLDING",
        "FOUR_HOUR_EMA10_HOLDING",
        oneHourWeak ? "ONE_HOUR_WEAK" : null,
        priceAbove1hEma10 === false ? "PRICE_BELOW_1H_EMA10" : null,
        structureState === "FAILURE" ? "SHORT_TERM_STRUCTURE_FAILURE" : null,
        "NO_BLIND_SHORTS",
        "W4_NOT_OFFICIALLY_CONFIRMED",
      ].filter(Boolean),
    };
  }  
...
function detectMinuteW4ABC({
  ...
  const trendState4h = upper(engine16?.trendState_4h);
  ...
  if (hasALow && !hasBHigh) {
    ...
    if (shallowContinuationWatch) {
      return {
        ...
        type: "W4_SHALLOW_CONTINUATION_WATCH",
        ...
        allowShortEntry: false,
        triggerConfirmed: false,
        triggerType: "WAIT_FOR_B_HIGH_OR_CONTINUATION_LEVEL",
        triggerLevel: null,
        ...
        debug: {
          ...
          note:
            "Official Engine 2 minute phase remains IN_W4. Engine 22 is only flagging shallow W4 continuation behavior.",
        },
      };
    }
  ...
  return {
    ...
    if (bBounceReady) {
      return {
        ...
        confidence: 55,
        sizeMode: "REDUCED",
        needs: "WAIT_FOR_B_BOUNCE_CONFIRMATION",
        marketBias,
        reasonCodes: [
          "MINUTE_W4_ACTIVE",
          "A_LOW_MARKED",
  ...
  if (hasALow && hasBHigh && !hasCLow) {
    ...
    if (shallowTrigger) {
      return {
        ...
        allowShortEntry: false,
        triggerConfirmed: true,
        triggerType: "SHALLOW_W4_BREAK_ABOVE_B_HIGH",
        triggerLevel: round2(bHigh),
        stopLevel: round2(aLow),
        confidence: marketFrontVeryStrong ? 76 : 70,
        sizeMode: "CAUTION",
        needs: "ENTRY_ACTIVE",
        marketBias,
        reasonCodes: [
        ...
        debug: {
          ...
          note:
            "Engine 22 shallow trigger only. Engine 2 should still require official MARK,W4 for official wave confirmation.",
        },
      };
    }
    ...
    if (shallowContinuationWatch) {
      return {
        ...
        triggerLevel: round2(bHigh),
        stopLevel: round2(aLow),
        confidence: marketFrontVeryStrong ? 68 : 62,
        sizeMode: "REDUCED",
        needs: "BREAK_B_HIGH_FOR_W5_SHALLOW_TRIGGER",
        marketBias,
        reasonCodes: [
          "MINUTE_W4_ACTIVE",
          "A_LOW_MARKED",
        ...
        debug: {
          ...
          note:
            "C low is not marked. Engine 22 is watching shallow W4 continuation because market front remains strong.",
        },
      };
    }
    ...
    if (bBounceStrong) {
      return {
        ...
        allowShortEntry: false,
        triggerConfirmed: false,
        triggerType: "B_BOUNCE_ACTIVE_WAIT_RESOLUTION",
        triggerLevel: round2(bHigh),
        stopLevel: round2(aLow),
        confidence: 58,
        sizeMode: "REDUCED",
...
function normalizeW4ABCForEngine17(detection) {
  ...
  const isEntryState =
    rawState === "A_TO_B_TRIGGER_LONG" ||
    rawState === "W5_TRIGGER_LONG" ||
    rawState === "W5_SHALLOW_TRIGGER_LONG";
...
function isStrongEngine22State(out) {
  if (!out || typeof out !== "object") return false;

  const state = upper(out.state || out.type);
  const status = upper(out.status);
  const setupType = upper(out.setupType);

  if (status === "ENTRY_LONG" || status === "ENTRY_SHORT") return true;
  if (status === "NO_SHORT") return true;

  return [
    "NEGOTIATED_ZONE_LOST",
    "NEGOTIATED_ZONE_REJECTION_WARNING",
    "FINAL_IMPULSE_NO_SHORTS",
    "SHORT_BLOCKED_BY_TREND_VS_WAVE",
    "SHORT_BLOCKED_BY_MARKET_BIAS",
    "EXHAUSTION_REJECTION_SHORT",
    "EXHAUSTION_BOUNCE_LONG",
    "DIP_BUY_CONTINUATION",
    "W5_SHALLOW_TRIGGER_LONG",
    "A_TO_B_TRIGGER_LONG",
    "W5_TRIGGER_LONG",
    "W3_DIP_BUY_TRIGGER_LONG",
    ...
    "EXHAUSTION_BOUNCE_LONG",
    "DIP_BUY_CONTINUATION",
    "W3_DIP_BUY_CONTINUATION",
    "W4_TO_W5_SHALLOW_CONTINUATION",
    "SHORT_BLOCKED_BY_MARKET_BIAS",
  ].includes(setupType);
}

function detectMicroW3ContinuationState({
  engine2State,
  engine16,
  marketBias,
  latestClose,
  ...
  const scalpExtension = engine2State?.activeExtensions?.scalp || null;

  const minutePhase = phases.minutePhase;
  const microPhase = phases.microPhase;

  const isMicroW3Context =
    minutePhase === "IN_W5" &&
    microPhase === "IN_W3";
  ...
  const commonDebug = {
    ...
    scalpExtension,
    note:
      "Micro W3 debounce v1 uses snapshot latestClose. TODO: upgrade to true two-closed-candle debounce once Engine 16 exposes recent completed 10m bars.",
  };

  if (!isMicroW3Context) {
    return {
      active: false,
      state: "NO_MICRO_W3_CONTEXT",
      reasonCodes: ["NOT_MICRO_W3_CONTEXT"],
      debug: commonDebug,
    };
  }

  // Real failure: 1H EMA10 lost.
  if (oneHourBelowEma10) {
    return {
      active: true,
      setupType: "MICRO_W3_INSIDE_MINUTE_W5",
      type: "MICRO_W3_FAILED",
      ...
      confidence: 0,
      sizeMode: "NONE",
      needs: "STAND_DOWN_1H_EMA10_LOST",
      marketBias,
      reasonCodes: [
        "MINUTE_W5_ACTIVE",
        "MICRO_W3_ACTIVE",
  ...
  // Stronger caution: 10m EMA20 lost.
  if (below10mEma20) {
    return {
      active: true,
      setupType: "MICRO_W3_INSIDE_MINUTE_W5",
      type: "MICRO_W3_WEAKENING",
      ...
      allowShortEntry: false,
      triggerConfirmed: false,
      confidence: continuationWatchLong ? 42 : 35,
      sizeMode: "NONE",
      needs: "WAIT_FOR_10M_EMA_RECLAIM",
      marketBias,
      reasonCodes: [
        "MINUTE_W5_ACTIVE",
        "MICRO_W3_ACTIVE",
        "PRICE_BELOW_10M_EMA20",
        "MICRO_W3_WEAKENING",
        "OBSERVATION_ONLY",
      ],
      debug: commonDebug,
  ...
  // Normal pullback test: EMA10 lost/touched, EMA20 still holding.
  if (belowOrAt10mEma10 && above10mEma20) {
    return {
      active: true,
      setupType: "MICRO_W3_INSIDE_MINUTE_W5",
      type: "MICRO_W3_PULLBACK_TEST",
      ...
      allowShortEntry: false,
      triggerConfirmed: false,
      confidence: continuationWatchLong ? 60 : 55,
      sizeMode: "CONTROLLED_DIP_ONLY",
      needs: "HOLD_10M_EMA20_OR_RECLAIM_10M_EMA10",
      marketBias,
      reasonCodes: [
        "MINUTE_W5_ACTIVE",
        "MICRO_W3_ACTIVE",
        "FIRST_10M_EMA10_TEST",
        "PRICE_ABOVE_10M_EMA20",
        oneHourAboveEma10 ? "ONE_HOUR_EMA10_HOLDING" : "ONE_HOUR_EMA10_NOT_CONFIRMED_LOST",
        continuationWatchLong ? "CONTINUATION_WATCH_LONG" : null,
        "MICRO_W3_PULLBACK_TEST",
        "OBSERVATION_ONLY",
      ].filter(Boolean),
      debug: commonDebug,
    };
  ...
  if (above10mEma10 && above10mEma20) {
    return {
      ...
      allowShortEntry: false,
      triggerConfirmed: false,
      confidence: continuationTriggerLong ? 72 : continuationWatchLong ? 68 : 65,
      sizeMode: "CONTROLLED_DIP_ONLY",
      needs: "BUY_CONTROLLED_DIPS_DO_NOT_CHASE",
      marketBias,
      reasonCodes: [
        "MINUTE_W5_ACTIVE",
        "MICRO_W3_ACTIVE",
        "PRICE_ABOVE_10M_EMA10_20",
        oneHourAboveEma10 ? "ONE_HOUR_EMA10_HOLDING" : "ONE_HOUR_EMA10_NOT_CONFIRMED_LOST",
        continuationTriggerLong ? "CONTINUATION_TRIGGER_LONG" : null,
        continuationWatchLong ? "CONTINUATION_WATCH_LONG" : null,
        "BUY_CONTROLLED_DIPS_ONLY",
        "OBSERVATION_ONLY",
      ].filter(Boolean),
      debug: commonDebug,
    };
  ...
  return {
    active: true,
    setupType: "MICRO_W3_INSIDE_MINUTE_W5",
    type: "MICRO_W3_ACTIVE_WAIT",
    state: "MICRO_W3_ACTIVE_WAIT",
    status: "WATCH",
    readiness: "WATCH",
    direction: "LONG",
    side: "LONG",
    ...
    confidence: 0,
    sizeMode: "NONE",
    needs: "WAIT_FOR_MICRO_W3_STRUCTURE",
    marketBias,
    reasonCodes: [
      "MINUTE_W5_ACTIVE",
      "MICRO_W3_ACTIVE",
...
function applyMicroContinuationPromotion(out, microContinuation) {
  ...
  const micro =
    microContinuation?.active === true
      ? microContinuation
      : null;

  const withMicro = {
    ...out,
    microContinuation: micro,
    debug: {
      ...(out.debug || {}),
      microContinuation: micro
        ? {
...
function buildCurrentLifecycleState({ engine22WaveStrategy }) {
  ...
  const waveOpportunity = engine22WaveStrategy?.waveOpportunity || engine22WaveStrategy || null;
  ...
  if (
    ...
    postDownImpulseBounce?.state === "POST_MINOR_5_BOUNCE_EXCEEDED_C2618_POSSIBLE_W5_UP"
  ) {
    return {
...
const engine22WaveStrategyDraft = {
  ...withMicroW4,
  regimeLayers,
  reactionContext,
  volumeContext,
...
const currentLifecycleState = buildCurrentLifecycleState({
  engine22WaveStrategy: engine22WaveStrategyDraft,
});
