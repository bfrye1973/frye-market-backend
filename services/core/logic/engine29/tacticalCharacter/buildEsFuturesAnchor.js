...
import { ENGINE29_EVIDENCE_QUALITY, ENGINE29_STRESS_DIRECTIONS } from "../constants.js";
...
function marketEntry({ hourly, thirtyMinute, now }) {
  ...
  return {
    ...
    subgroup: "ES_FUTURES",
    // Structure classifier requires a stress direction, but ES anchor classification is NOT
    // used as a trade-direction command. Move character reads price displacement/sweeps.
    stressDirection: ENGINE29_STRESS_DIRECTIONS.LOWER,
    required: true,
    provider: "FRYE_FUTURES_OHLC_PROVIDER",
    sourceSymbol: thirtyMinute?.resolvedSymbol || hourly?.resolvedSymbol || null,
    sourceSeriesId: null,
    isProxy: false,
    proxyFor: null,
    evidenceQuality: ENGINE29_EVIDENCE_QUALITY.DIRECT,
    available: Boolean(oneHourBars.length || thirtyMinuteBars.length),
    structural: null,
    tactical: {
      timeframe: "1H",
      sourceTimeframe: "1H",
...
export async function buildEngine29EsFuturesAnchor({
  ...
  const [hourly, thirtyMinute] = await Promise.all([
    fetchFuturesBars({ symbol, timeframe: "1h", limit: oneHourLimit }),
    fetchFuturesBars({ symbol, timeframe: "30m", limit: thirtyMinuteLimit }),
  ]);

  const entry = marketEntry({ hourly, thirtyMinute, now });
  const structure = buildEngine29SymbolStructure(entry, { now });

  return {
    version: "engine29.esAnchor.v1",
    timestamp: new Date(now).toISOString(),
    productCode: symbol,
    resolvedSymbol: thirtyMinute?.resolvedSymbol || hourly?.resolvedSymbol || null,
    source: "FRYE_FUTURES_OHLC_PROVIDER",
    resolver: thirtyMinute?.resolver || hourly?.resolver || null,
