...
function plainGroupLabel(key, state, group) {
  if (
    key === "volatility" &&
    !state &&
    group?.structural?.missingRequiredMembers?.includes("VIX_DIRECT")
  ) {
...
export async function buildEngine29CrossMarketStress({
  ...
  const market =
    marketDataBundle ||
    await buildEngine29MarketDataBundle({ now });

  const structure =
    structureBundle ||
    buildEngine29StructureBundle(market, { now });

  const groups =
    groupBundle ||
    buildEngine29GroupStateBundle(structure, {
      now,
      financialConditions,
    });
