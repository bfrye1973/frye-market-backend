...
function plainGroupLabel(key, state, group) {
  if (key === "volatility" && !state && group?.structural?.missingRequiredMembers?.includes("VIX_DIRECT")) return "NO DIRECT VIX FEED";
  if (key === "financialConditions" && !state) return "NOT ACTIVE YET";
  ...
  if (state === "FORMING") {
    if (key === "ratesDuration") return "PRESSURE BUILDING";
    if (key === "energyInflation") return "STRESS BUILDING";
    if (key === "credit") return "WEAKENING";
    if (key === "headlineIndex") return "WEAKENING";
    return "WEAKENING";
  }
...
function structuralSummary(state) {
  if (state === "NORMAL") return "Cross-market conditions are broadly healthy.";
  if (state === "EARLY_WARNING") return "Early weakness is appearing, but stress is not yet broad.";
  if (state === "BROAD_DETERIORATION") return "Weakness is confirmed across multiple independent parts of the market.";
  if (state === "RISK_OFF_CONFIRMED") return "Credit and volatility have joined broad market deterioration.";
  if (state === "SYSTEMIC_STRESS") return "Stress is broad across indexes, internals, credit, volatility and financial conditions.";
  return "Not enough reliable evidence for a structural state.";
}

function tacticalSummary(state) {
  if (state === "NORMAL") return "Intraday cross-market conditions are calm.";
  if (state === "CAUTION") return "Intraday pressure is present under the surface.";
  if (state === "RISK_OFF_ACTIVE") return "Multiple 1-hour groups are confirming active risk-off pressure.";
  if (state === "STRESS_ACCELERATING") return "1-hour stress is active and the faster ES move is accelerating lower.";
  if (state === "RECOVERING") return "ES is repairing intraday while broader confirmation remains limited.";
...
export async function buildEngine29CrossMarketStress({
  ...
  const market = marketDataBundle || await buildEngine29MarketDataBundle({ now });
  const structure = structureBundle || buildEngine29StructureBundle(market, { now });
  ...
  for (const [key, state] of Object.entries(structuralStates)) {
    displayGroups[key] = plainGroupLabel(key, state, groups?.groups?.[key]);
  }

  return {
    version: `${ENGINE29_VERSION}.phase5`,
    timestamp: new Date(now).toISOString(),
    dataDegraded: Boolean(market?.dataDegraded) || Boolean(groups?.dataDegraded) || Boolean(move?.dataDegraded),

    overallState: structural.state,
    structuralState: structural.state,
    tacticalState: tactical.state,
    fastTacticalState: fastTactical.state,
    esNqBackdrop: structural.backdrop,
    ...
    warnings: structural.activeGroups.filter((key) => !structural.confirmedGroups.includes(key)),
    ...
    display: {
      ...
      underTheHood: {
        ...
        financialConditions: displayGroups.financialConditions,
        pressure: move?.display?.underlyingPressure ?? null,
      },
      missingConfirmation: missingConfirmations,
    },
  };
}
