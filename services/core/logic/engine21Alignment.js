...
function getDirection({ price, ema10, ema20, symbol }) {
  ...
  if (isBull) return "BEAR_CONFIRM";   // ↑ volatility = bearish market
  return "NEUTRAL";
}
...
export async function computeEngine21Alignment({ tf = DEFAULT_TF } = {}) {
  ...
  for (const symbol of SYMBOLS) {
    try {
     ...
     if (symbol === "VIX" || symbol === "UVXY") {
       if (direction === "BULL_CONFIRM") bullishScore += SCORE_PER_COMPONENT;
       if (direction === "BEAR_CONFIRM") bearishScore += SCORE_PER_COMPONENT;
     } else {
