...
function normalizeZone(zone) {
  ...
  const lo = toNum(zone.lo ?? zone.low ?? zone.from ?? zone.lower ?? zone.bottom);
  const hi = toNum(zone.hi ?? zone.high ?? zone.to ?? zone.upper ?? zone.top);
  ...
  return {
    ...
    lo: low,
    hi: high,
    mid: toNum(zone.mid ?? zone.midline) ?? round2((low + high) / 2),
  };
}
...
function buildInactive({
  ...
  return {
    ...
    quality: "WEAK",
    direction: "NEUTRAL",
    tradeDirectionBias: safeUpper(handoff?.tradeDirectionBias, "NONE"),
...
function validateIdentity({ handoff, expectedIdentity = null }) {
  ...
  if (!hasValidIdentity(identity)) {
    return { ok: false, identity, reason: "ENGINE26_REACTION_HANDOFF_IDENTITY_MISSING" };
  }
  if (expectedIdentity && typeof expectedIdentity === "object") {
    ...
    if (
      (expectedCandidateId && identity.candidateId !== expectedCandidateId) ||
      (expectedZoneId && identity.zoneId !== expectedZoneId)
    ) {
      return { ok: false, identity, reason: "ENGINE26_REACTION_HANDOFF_IDENTITY_MISMATCH" };
    }
  }
...
function inferFactBooleans({ handoff, entryZone }) {
  ...
  const lowestRemainedAboveBoundary =
    ...
    closeFacts.lowestPriceSinceLatestReclaim == null
      ? true
      : closeFacts.lowestPriceSinceLatestReclaim >= Number(handoff.locationInvalidationBoundary);
  return {
...
function deriveObservedReaction({ facts }) {
  ...
  if (facts.closedAboveMidline && facts.reclaimObserved) return "RECLAIMED_LEVEL";
  if (facts.meaningfulLowerWick && facts.reclaimObserved) return "WICK_BELOW_AND_RECLAIM";
...
function isExpected({ observedReaction, expectedReactions }) {
  ...
  if (observedReaction === "WICK_RECLAIM_OBSERVED" && expectedReactions.includes("WICK_BELOW_AND_RECLAIM")) return true;
  if (observedReaction === "RECLAIMED_LEVEL" && expectedReactions.includes("HELD_LEVEL")) return true;
  return false;
}

function qualityFor({ reactionState, facts, expectedMatch, intrabarDowngrade }) {
  if (reactionState === "WAITING_FOR_ZONE_INTERACTION" || reactionState === "SWEEP_OBSERVED") return "WEAK";
  if (reactionState === "REACTION_INVALIDATED") return "WEAK";
  if (intrabarDowngrade) return "WEAK";
  if (!expectedMatch) return "WEAK";
  if (facts.closedAboveZoneHigh || (facts.reclaimObserved && facts.oneHoldClose && facts.closedAboveMidline && facts.lowestRemainedAboveBoundary)) return "STRONG";
  if (facts.reclaimObserved || facts.meaningfulLowerWick || facts.closedAboveZoneLow) return "GOOD";
  return "WEAK";
}

export function buildEngine26Strategy1Reaction({ engine26ReactionHandoff = null, expectedIdentity = null } = {}) {
  ...
  const identityCheck = validateIdentity({ handoff, expectedIdentity });
  if (!identityCheck.ok) {
    return buildInactive({
      handoff,
  ...
  if (facts.completedCloseInvalidationConfirmed) {
    return {
      ...buildInactive({
        ...
        reasonCodes: ["COMPLETED_CLOSE_INVALIDATION_CONFIRMED", "REACTION_INVALIDATED"],
  ...
  const expectedMatch = isExpected({ observedReaction, expectedReactions });
  ...
  } else if (facts.reclaimObserved && !facts.oneHoldClose && !facts.closedAboveMidline) {
    reactionState = "RECLAIM_OBSERVED";
    authorizedReactionState = "RECLAIM_HOLD_DEVELOPING";
  } else if (facts.reclaimObserved && facts.oneHoldClose && !facts.closedAboveMidline) {
    reactionState = "RECLAIM_HOLD_DEVELOPING";
    authorizedReactionState = "RECLAIM_HOLD_DEVELOPING";
  } else if (expectedMatch && facts.reclaimObserved && (facts.twoHoldCloses || facts.closedAboveMidline || facts.closedAboveZoneHigh) && facts.lowestRemainedAboveBoundary) {
  ...
  const intrabarDowngrade = facts.intrabarInvalidationBreach === true;
  const quality = qualityFor({ reactionState, facts, expectedMatch, intrabarDowngrade });
  const direction = safeUpper(handoff.tradeDirectionBias, "LONG") === "SHORT" ? "SHORT" : "LONG";
  const blockers = [
    !expectedMatch && observedReaction !== "WAITING_FOR_ZONE_INTERACTION" && observedReaction !== "SWEEP_OBSERVED"
      ? "REACTION_NOT_IN_AUTHORIZED_EXPECTED_SET"
      : null,
    intrabarDowngrade ? "INTRABAR_INVALIDATION_BREACH_QUALITY_DOWNGRADE" : null,
  ].filter(Boolean);
  ...
  return {
    ...
    quality,
    direction,
    tradeDirectionBias: safeUpper(handoff.tradeDirectionBias, direction),
    ...
    entryZone,
    targetZone,
    locationInvalidationBoundary: toNum(handoff.locationInvalidationBoundary) ?? toNum(handoff.invalidationLevel) ?? null,
    currentPrice: getCurrentPrice(handoff),
    ...
    reasonCodes: uniqueReasonCodes([
      ...
      facts.meaningfulLowerWick ? "MEANINGFUL_LOWER_WICK_OBSERVED" : null,
      ...
      intrabarDowngrade ? "INTRABAR_INVALIDATION_BREACH_QUALITY_DOWNGRADE" : null,
      reactionConfirmed ? "REACTION_CONFIRMED" : "REACTION_NOT_CONFIRMED",
      ...blockers,
      "NO_PERMISSION_CREATED",
      "NO_EXECUTION",
