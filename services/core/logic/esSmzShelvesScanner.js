// services/core/logic/esSmzShelvesScanner.js
// Engine 1B — ES Futures Smart Money Shelves Scanner (Acc/Dist)
//
// Based on SPY smzShelvesScanner.js, but ES-specific thresholds.
//
// Lock:
// - Detection timeframe = 30m primary
// - Confirmation timeframe = 1h secondary
// - No 15m detection for v1
//
// Output:
// { type, price, priceRange:[high,low], strength_raw (40..100), confidence (0..1), diagnostic:{...} }

const DEFAULT_BAND_POINTS = 80;

// shelf window scan (30m bars)
const WINDOW_SIZES = [4, 6, 8]; // ~2h, 3h, 4h consolidations
const SHELF_MIN_WIDTH = 3.0; // ES points
const SHELF_MAX_WIDTH = 18.0; // ES points
const BODY_TO_RANGE_MAX = 0.65;
const OVERLAP_RATIO_MIN = 0.55;

// wick + breakout confirmation
const ATR_PERIOD = 50;
const STRONG_WICK_ATR = 0.7;
const BREAK_EPS = 0.25; // ES tick-aware
const BREAK_LOOKAHEAD = 8; // 30m bars
const DISP_MIN_ATR = 0.45;
...
function rangeDistanceWeight(hi, lo, currentPrice, bandPoints) {
  ...
  if (!(Number.isFinite(H) && Number.isFinite(L) && Number.isFinite(currentPrice))) {
    return 0;
  }
...
function tfConfirmBoost(hi, lo, bars1h) {
  const margin = Math.max(2.0, (hi - lo) * 0.6);
  const zHi = hi + margin;
  const zLo = lo - margin;
...
function detectBreakout(bars, endIdx, hi, lo, center, atr) {
  ...
  return maxUp >= maxDown
    ? { dir: "up", moveATR: maxUp }
    : { dir: "down", moveATR: maxDown };
}
...
export function computeShelves({ bars30m, bars1h, bandPoints = DEFAULT_BAND_POINTS }) {
  ...
  for (const win of WINDOW_SIZES) {
    for (let endIdx = win - 1; endIdx < b30.length; endIdx++) {
      ...
      const baseWidthScore =
        1 - (width - SHELF_MIN_WIDTH) / (SHELF_MAX_WIDTH - SHELF_MIN_WIDTH);
      ...
      candidates.push({
        ...
        _diag: {
          ...
          meta: {
            ...
            bandPoints,
            atr: round2(atr),
            scanner: "ES_ENGINE_1B_SCANNER_V1",
          },
        },
      });
    }
  ...
  const merged = candidates.filter((s) =>
    overlapsBand(
      Math.max(s.priceRange[0], s.priceRange[1]),
      Math.min(s.priceRange[0], s.priceRange[1]),
      bandLow,
      bandHigh
    )
  );
