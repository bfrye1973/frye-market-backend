...
test(
  ...
  () => {
    ...
    try {
      ...
      assert.deepEqual(
      restored.expectedReactions,
      []
    );
      assert.equal(
      restored.reactionExpected,
      false
    );
...
test(
  "stale Strategy 1 identity does not arm an inactive candidate handoff",
  () => {
    const handoff = buildEngine26ReactionHandoff({
      locationCandidate: {
        active: false,
        ...
        strategyId: "intraday_scalp@10m",
        timeframe: "10m",
        snapshotTime: "2026-08-06T15:50:00.000Z",
        directionBias: "NEUTRAL",
        direction: "NEUTRAL",
        strategyEligibility: {
          eligible: true,
    ...
    assert.equal(handoff.candidateIdentityValid, true);
    assert.equal(handoff.candidateActive, false);
    assert.equal(
      handoff.status,
...
test(
  "Engine 26 observer stays active while candidate handoff remains inactive without valid location context",
  () => {
    const result =
      buildEngine26A({
        symbol: "ES",
  ...
  () => {
    ...
    try {
      fs.writeFileSync(
        manualZonesFilePath,
        [
          "7675.00-7715.00 | NEG 7687.25-7705.50",
          "",
        ].join("\n"),
        "utf8"
      );
  ...
  () => {
    ...
    try {
      ...
      const matured = buildAtPrice({
        currentPrice: 7712,
        previousLocationCandidate: first,
        snapshotTime:
...
test(
  ...
  () => {
    const tempDir = fs.mkdtempSync(
      path.join(
        os.tmpdir(),
        "engine26-engine22-no-inference-"
      )
    );
