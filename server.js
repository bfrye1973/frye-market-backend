...
const GAUGE_GROUPS = (process.env.GAUGE_GROUPS || "")
  .split(",").map(s=>s.trim()).filter(Boolean);
...
function parseCsvSimple(text){
  ...
  const rows = []; const nowYear = new Date().getFullYear();
...
// READ endpoints
app.get("/api/schwab/accountNumbers", async (req,res)=>{
